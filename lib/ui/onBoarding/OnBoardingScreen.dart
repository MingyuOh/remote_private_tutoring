...
List<Widget> populatePages(BuildContext context) {
  ...
  _titlesList.asMap().forEach((index, value) => _pages.add(getPage(
      image: _imageList.elementAt(index),
      title: value,
      subTitle: _subtitlesList.elementAt(index),
      context: context,
      isLastPage: _isLastPage(index + 1, _titlesList.length))));
  return _pages;
}
...
Widget getPage({String image, String title, String subTitle,
  ...
  return Stack(
    children: <Widget>[
      ...
      // -------------------- Start --------------------
      // OnboardingPage에서 마지막 페이지에 표시되는 'Continue'버튼
      Visibility(
        visible: isLastPage,
        child: Padding(
            padding: const EdgeInsets.all(16.0),
